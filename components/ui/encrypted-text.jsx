...
function generateDeterministicCharacter(original, index, charset) {
  const seed = (original.charCodeAt(0) + 17) * (index + 23);
  return charset.charAt(seed % charset.length);
}

function generateDeterministicGibberishPreservingSpaces(original, charset) {
  if (!original) return "";
  let result = "";
...
export const EncryptedText = ({
  ...
  const scrambleCharsRef = useRef(
    text ? generateDeterministicGibberishPreservingSpaces(text, charset).split("") : []
  );
